#!/bin/bash
# Quick check of repository access for Shipyard RPM dependencies
#
...
echo "==============================="
echo
echo -e "${B}Component  Packages                        Repository        x86_64  aarch64 ppc64le s390x${N}"
echo "---------  ------------------------------  ----------------  ------  ------- ------- -----"
...
# nettest: other packages from UBI (public)
printf "nettest    bind-utils,curl,iproute,...     UBI (public)      "
for arch in x86_64 aarch64 ppc64le s390x; do
    if check_public "$UBI/$arch/appstream/os/repodata/repomd.xml"; then
        printf "${G}%-8s${N}" "OK"
    else
        printf "${R}%-8s${N}" "N/A"
    fi
