...
#   .rpm-lockfiles/verify-packages.sh release-0.22    # Verify specific branch
#
...
podman run --rm \
    ...
    registry.access.redhat.com/ubi9/ubi:latest bash -c '
        ...
        echo -e "${B}Note:${N} iperf3 and tcpdump are NOT in UBI repos - only in RHEL AppStream."
        echo "      RHEL repos require entitlements that currently lack ppc64le/s390x."
    '
