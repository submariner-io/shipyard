...
update_component_lockfile() {
  ...
  # Validate containerfile path exists
  local containerfile_path=$(grep "containerfile:" "${REPO_ROOT}/${lockfile_dir}/rpms.in.yaml" 2>/dev/null | sed 's/.*containerfile: *//' || echo "")
  if [ -n "$containerfile_path" ]; then
    local full_containerfile="${REPO_ROOT}/${lockfile_dir}/${containerfile_path}"
    if [ ! -f "$full_containerfile" ]; then
      echo "ERROR: Containerfile not found for ${component}"
      echo "Expected: ${full_containerfile}"
      echo "Check containerfile path in ${lockfile_dir}/rpms.in.yaml"
      return 1
    fi
  fi
