...
# This script has been copied from https://github.com/kubernetes/kubernetes/blob/master/hack/cherry_pick_pull.sh
# and edited to suit the need.

# Checkout a PR from GitHub. (Yes, this is sitting in a Git tree. How
# meta.) Assumes you care about pulls from remote "my-fork" and
# checks them out to a branch named:
#  automated-backport-of-<pr>-<target branch>-<timestamp>

# Usage Instructions: https://submariner.io/development/backports/
set -o errexit
set -o nounset
set -o pipefail
...
FORK_REMOTE=${FORK_REMOTE:-$GITHUB_USER}
...
if [[ "$#" -lt 2 ]]; then
  echo "Usage: make backport release=<release branch> pr=<pr to backport>"
  echo "${0} <remote branch> <pr-number>...: backports one or more PRs by cherry picking onto <remote branch> and proposes a new pull request, if desired"
  echo
  echo "  Checks out <remote branch> and handles the cherry-pick of <pr> (possibly multiple) for you."
  echo "  Examples:"
  echo "    make backport release=release-3.14 pr=12345        # Cherry-picks PR 12345 onto origin/release-3.14 and proposes that as a PR."
  echo "    make backport release=release-3.14 pr=12345,56789  # Cherry-picks PR 12345, then 56789 and proposes the combination as a single PR."
  echo
  ...
  echo
  echo "  Set UPSTREAM_REMOTE (default: origin) and FORK_REMOTE (default: $GITHUB_USER)"
...
declare -r BRANCH="${UPSTREAM_REMOTE}/$1"
shift 1
IFS=',' read -ra PULLS <<< "$@"
...
NEWBRANCHREQ="automated-backport-of-${PULLDASH}" # "Required" portion for tools.
